...
function onValueChanged(key, value, isNew) {
	...
	switch (key) {
		case 'drive/navx/yaw': // Gyro rotation
			ui.gyro.val = value;
	...
	// This section displays a list of all NetworkTables variables (that start with ) and allows you to directly manipulate them.
	var propName = key.substring(16, key.length);
	// Check if value is new and doesn't have a spot on the list yet
	if (isNew && !document.getElementsByName(propName)[0]) {
		// Make sure name starts with . Properties that don't are technical and don't need to be shown on the list.
		if (key.substring(0, 16) === '') {
...
ui.example.button.onclick = function() {
	...
	NetworkTables.setValue('example_variable', this.className != 'active');
};
...
ui.gyro.container.onclick = function() {
	...
	onValueChanged('drive/navx/yaw', ui.gyro.val);
};
...
ui.autoSelect.onchange = function() {
	NetworkTables.setValue('autonomous/selected', this.value);
};

// Get value of arm height slider when it's adjusted
ui.armPosition.oninput = function() {
	NetworkTables.setValue('arm/encoder', parseInt(this.value));
};
